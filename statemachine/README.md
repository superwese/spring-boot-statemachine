...
`
or change your .aws/config accordingly
...
```bash
createEvents$ sam logs -n StockCheckerFunction --stack-name createEvents --tail
```
...
```bash
createEvents$ cd functions/<function-name>/
...
```bash
aws cloudformation delete-stack --stack-name createEvents
```
